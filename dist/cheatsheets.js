...
(async () => {
  ...
  blocksForProduction.forEach(blockName => {
    ...
    if (blockName === "citation_styles") {
      document.getElementById("citation_styles-heading").innerHTML = "Help with Citing"; // console.log("citation_stylesINIT");
    ...
    if (blockName === "primo_book_searches") {
      document.getElementById("primo_book_searches-heading").innerHTML = "Select Books from RMC Library"; // console.log("primo_book_searchesINIT");
...
module.exports = async proxyPrepend => {
  ...
  prepDom.insertAdjacentHTML("beforeend", "<div id='excellent_for'></div>"); //  prepDom.insertAdjacentHTML("beforeend","<div id='good_for'>Good for</div>")
...
module.exports = (proxyPrepend, blockData) => {
  ...
  let domFinishing = searchTerms => {
    ...
    linksList.insertAdjacentHTML("afterend", `<div id="ebsco_api_a9h-afterThangs"><a href="http://ezproxy.rocky.edu:2048/login?url=https://search.ebscohost.com/login.aspx?profile=ehost&defaultdb=a9h&authtype=ip&custid=s8250848" target="_blank"><button class="btn">Run a new search</button></a></div>`);
    linksList.insertAdjacentHTML("beforebegin", `<div id="ebsco_api_a9h-beforeThangs"><span class="caps"> ${searchTerms}</span> </div>`);
  };
};
...
module.exports = async blockData => {
  ...
  let domFinishing = searchTerms => {
    ...
    linksList.insertAdjacentHTML("beforebegin", `<div id="primo_article_searches-beforeThangs"><span class="caps"> ${searchTerms}</span> </div>`);
    linksList.insertAdjacentHTML("afterend", `<div id="primo_article_searches-afterThangs"><a href="https://rocky-primo.hosted.exlibrisgroup.com/primo-explore/search?sortby=rank&vid=01TRAILS_ROCKY&lang=en_US&mode=advanced" target="_blank"><button class="btn">Run a new search</button></a></div>`);
  };
};
